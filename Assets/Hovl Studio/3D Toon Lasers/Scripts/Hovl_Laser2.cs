    ...
    void Update()
    {
        if (laserPS != null && UpdateSaver == false)
        {
            ...
            if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, MaxLength))
            {
                ...
                {
                    hit.collider.gameObject.GetComponent<Animation_Test>().isAttack = true;
                    if (hit.collider.gameObject.GetComponent<Animation_Test>().health <= 0)
                    {
                        hit.collider.gameObject.GetComponent<Animation_Test>().isDead = true;
                    }
                    else
                    {
                        hit.collider.gameObject.GetComponent<Animation_Test>().health = hit.collider.gameObject.GetComponent<Animation_Test>().health - 0.1f;
                ...
                {
                    hit.collider.gameObject.GetComponent<Animation_Test>().isAttack = false;
                }
